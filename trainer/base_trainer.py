...
class BaseTrainer(ABC):
    def __init__(self, config, run_path, distributed_meta_info, resume=False):
        super().__init__()
        ...
        self.local_rank = self.distributed_meta_info["local_rank"]
        ...
        if os.path.exists(os.path.join(os.path.join(self.run_path, 'checkpoints'), 'latest.pt')) and resume: # restore to train
            self._build_everything()
            self.load(os.path.join(os.path.join(self.run_path, 'checkpoints'), 'latest.pt'))
            if self.rank == 0:
                print("resuming...")
